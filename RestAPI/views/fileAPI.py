...
def GeneratePDF(request):
    ...
    if request.method == "GET":
        ...
        if transportType == "content":
            ...
            if NGF:
                if _url.split("/")[2] ==os.getenv('NGF_PROXY_ADDRESS_1'):
                    _url = _url.replace(os.getenv('NGF_PROXY_ADDRESS_1'),os.getenv('NGF_TARGET_ADDRESS'))
                elif _url.split("/")[2] == os.getenv('NGF_PROXY_ADDRESS_2'):
                    _url = _url.replace(os.getenv('NGF_PROXY_ADDRESS_2'),os.getenv('NGF_TARGET_ADDRESS'))
        else:
